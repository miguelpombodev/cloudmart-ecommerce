...
CREATE SCHEMA IF NOT EXISTS audit AUTHORIZATION identity;
...
BEGIN
	IF
NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'identity_readonly') THEN
...
-- identity_app reads and writes, but can´t affect the structure
GRANT
SELECT,
INSERT
,
UPDATE,
DELETE
ON ALL TABLES IN SCHEMA identity TO identity_app;
GRANT
USAGE,
SELECT
ON ALL SEQUENCES IN SCHEMA identity TO identity_app;


-- identity_readonly only reads
GRANT
SELECT
ON ALL TABLES IN SCHEMA identity TO identity_readonly;
...
DEFAULT PRIVILEGES IN SCHEMA identity
      GRANT USAGE,
SELECT
ON SEQUENCES TO identity_app;

ALTER
DEFAULT PRIVILEGES IN SCHEMA identity
      GRANT
SELECT
ON TABLES TO identity_readonly;

-- default timezone
ALTER
DATABASE "IdentityDB" SET timezone TO 'UTC';

-- Avoid accidental full seq scan em bigger tables
ALTER
DATABASE "IdentityDB" SET enable_seqscan TO on;
...
COMMENT
ON COLUMN identity.outbox_messages.type          IS 'AssemblyQualifiedName do tipo do evento para desserialização';
COMMENT
ON COLUMN identity.outbox_messages.processed_on  IS 'NULL indica mensagem pendente. Preenchido após publicação no broker';
COMMENT
ON COLUMN identity.outbox_messages.error         IS 'Preenchido quando a publicação falha após todas as tentativas';
